...
$sql = "SELECT 
        ...
        if ($fromdateEsc) {
            $sql .= " AND sp.created_datetime >= '$fromdateEsc 00:00:00' ";
        }
        
        if ($todateEsc) {
            $sql .= " AND sp.created_datetime <= '$todateEsc 23:59:59' ";
        }
        

        $sql .= " ORDER BY cr.cl_req_id ASC, days_taken DESC";
...
$pdf->SetTitle('Pending Details Report');
$pdf->AddPage();
...
$pdf->Cell(200, 10, 'Clearance Pending Details Report', 0, 1, 'L');
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $html .= '<tr>
        ...
        <td>' . $row['pending_created_date'] . '</td>
        <td>' . $row['last_pending_date'] . '</td>
...
// Output PDF (inline)
$pdf->Output('pending_report_' . date('Ymd') . '.pdf', 'D'); // 'I' for inline, 'D' for download
exit;
