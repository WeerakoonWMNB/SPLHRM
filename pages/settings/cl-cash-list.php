                  ...
                  <div class="card">
                    <div class="card-body">
                      ...
                      <button type="button" data-bs-toggle="modal" data-bs-target="#myModal" class="btn btn-success" onclick="add_branch()"><i class="mdi mdi-playlist-plus me-1"></i> Add Item</button>
                        <?php 
                            $stmt = $conn->prepare("SELECT cl_amount_items.*,branch_departments.bd_name FROM cl_amount_items 
                            LEFT JOIN branch_departments ON branch_departments.bd_id = cl_amount_items.bd_id 
                            WHERE cl_amount_items.status = 1");
                                        ...
                                        <?php
                                            ...
                                            foreach($branches as $branch)
                                            {
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <form method="POST" action="../../back/cl-cash-manage.php" style="display: flex; gap: 5px;">
                                                            <input type="hidden" name="cl_amount_item_id" value="<?= $branch['cl_amount_item_id'] ?>">
                                                            <button type="button" class="btn btn-warning btn-sm" onclick="branch_set(<?= $branch['cl_amount_item_id'] ?>)">
        ...
        <div class="row">
            <div class="col-sm-12 col-xl-12">
                <div class="bg-light rounded h-100 p-4">
                    <form id="myform" method="POST" action="../../back/cl-cash-manage.php" enctype="multipart/form-data">
...
<div id="customAlertSuccess" class="custom-alert-success"></div>
  ...
  <script>
    ...
    $.ajax({
        url: '../../back/cl-cash-manage.php', // Replace with your endpoint that fetches user details
        type: 'POST',
